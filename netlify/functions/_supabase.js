...
function mapUser(row) {
  ...
  return {
    ...
    clientKey: row.role === 'client' ? (row.client_key || row.client_name || row.name || row.username || '') : (row.client_key || ''),
    clientName: row.role === 'client' ? (row.client_name || row.client_key || row.name || row.username || '') : (row.client_name || row.client_key || ''),
